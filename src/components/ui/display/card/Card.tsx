...
const Body = ({ children, className = "" }: cardProps) => {
    ...
    return (
        <div className={`p-6 w-fit flex flex-col lg:flex-row ${justifyClass} gap-8 items-center ${className}`}>
            {children}
        </div>
    )
}
...
const Image = ({ children, className = "", shape = "circle" }: cardProps) => {
    const baseStyle = "aspect-square shrink-0 overflow-hidden flex justify-center items-center";
    const shapeClass = shape === "circle" ? "w-[300px] rounded-full" : "h-[200px] w-[300px] rounded-2xl";
...
const Description = ({ children, className = "" }: cardProps) => {
    return (
        <div className={`flex flex-col gap-4 sm:max-w-2xl ${className}`}>
            {children}
        </div>
    )
}
